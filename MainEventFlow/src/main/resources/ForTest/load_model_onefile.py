...
scaler_cell = pickle.load(open(model_directory + 'scaler_cell.pkl', 'rb'))
...
def predict(xinputs_df):
    xinput_cell = xinputs_df.iloc[:,0:90]
    xinput_ccp= xinputs_df.iloc[:,90:99]

    xinput_cell_scaled = scaler_cell.transform(xinput_cell)
    xinput_ccp_scaled = scaler_ccp.transform(xinput_ccp)
